...
void create_polynomial(struct term** poly, int coef, int pow)
{
	...
	if(temp1==NULL)
	{
		temp2 = (struct term*)malloc(sizeof(struct term));	//Dynamic node creation 
    	temp2->coef = coef;
		temp2->pow = pow;
		//Updating the null pointer with the address of the first node of the polynomial just created
		*poly=temp2;
	...
	{
		temp2->next = (struct term*)malloc(sizeof(struct term));	//Dynamic node creation
...
void poly_add(struct term** pol, struct term* poly1, struct term* poly2)
{
    //Creating a temporary linked list to store the resultant polynomial
 	...
 	temp->next = NULL;
 	*pol = temp;    //*pol always points to the 1st node of the resultant polynomial
    ...
    //until one gets exhausted
 	while(poly1 && poly2)
 	{
        ...
        */
        ...
        if (poly1 && poly2)
        {
            temp->next = (struct term*)malloc(sizeof(struct term));		//Dynamic node creation
