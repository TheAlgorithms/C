/**
 * @file
 * @brief An [affine cipher](https://en.wikipedia.org/wiki/Affine_cipher) is a letter substitution cipher that uses a linear transformation to substitute letters in a message.
 * @details Given an alphabet of length M with characters with numeric values 0-(M-1), an arbitrary character x can be transformed with the expression (ax + b) % M into our ciphertext character. The only caveat is that a must be relatively prime with M in order for this tranformation to be invertible, i.e., gcd(a, M) = 1.
...
/**
 * @brief Given a valid affine cipher key, this function will produce the inverse key.
 *
...
void tests()
{
	...
	test_string("One-1, Two-2, Three-3, Four-4, Five-5, Six-6, Seven-7, Eight-8, Nine-9, Ten-10", 51, 18);
